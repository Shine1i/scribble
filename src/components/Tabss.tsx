...
export const Tabss = () => {
    ...
    return (
        <div className="flex flex-col  text-[#cccccc] font-mono">
            <div className="flex bg-[hsl(var(--editor-background))] overflow-x-auto divide-x divide-border">
                {tabs.map((tab) => (
                    <div
                        key={tab.id}
                        className={`flex items-center px-3 py-1 cursor-pointer  ${
                            tab.id === activeTab ? 'bg-card border-t !border-primary !border-x-0' : 'bg-[hsl(var(--editor-background))] hover:bg-card'
                        }`}
            ...
            {activeTabItem && (
                <div
                    className="flex items-center shadow-md shadow-slate-900/20 border-t border-slate-900/20  bg-[hsl(var(--editor-background))] text-[#cccccc] text-xs px-2 py-1">
